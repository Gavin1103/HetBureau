...
class FormulierF3 extends Formulieren
{
    ...
    public function showStudentBlockF3()
    {
        if (isset($_GET["leerlingNummer"])) {
            ...
            echo "<div class='boxtext F3'><a href='../forms/fulledInForm_F3.php?formNumber=form2&leerlingNummer={$this->_leerlingnummer}'>Fase 2 Assessmentformulier</a></div>";
        } else {
            header("location: ../admin/?error=geenstudentNummer");
        }
    }
    public function showFormF3()
    {
...
class sendFormF3 extends Formulieren
{
    private $student;
    ...
    public function checkIfStudentExistF3()
    {
        $db = new Database();
            ...
            if (!empty($_POST["student_name_af3"]) && !empty($_POST["coach_name_af3"]) && !empty($_POST["leerlingNummer_af3"]) && !empty($_POST["klas_name_af3"]) && !empty($_POST["datum_name_af3"])) {
                ...
                if ($verifyStudent->execute()) {
                    ...
                    if (is_array($result)) {
                        ...
                        header("location: ../forms/formulieren_fase_3.php?formNumber=form1&&error=studentExist");
                    } else {
    ...
    private function existStudent()
    {
        ...
        $this->leerlingNummer = $db->con->real_escape_string($_POST["leerlingNummer_af3"]);
    ...
    private function addStudent()
    {
        $db = new Database();
        $this->leerlingNummer = $db->con->real_escape_string($_POST["leerlingNummer_af3"]);
        $this->naam = $db->con->real_escape_string($_POST["student_name_af3"]);
        // $this->existStudent();
        $db = new Database();
...
// CRUD systeem 
class editFormF3 extends Formulieren
{
    function editForm_AF3()
    {
