...
class sendForms
{
    ...
    function editForm_AF1()
    {
        if (isset($_POST["edit_form_AF1"])) {
            ...
            $db = new Database();
            $this->mysqliQRY = mysqli_query($db->con, "UPDATE `opgeslagen_form_af1` SET `student`='$this->student',`coach`='$this->coach',`klas`='$this->klas',`checkboxen`='$trimArray',`vormgeven_veld`='$this->vormgeven',`techniek_veld`='$this->techniek',`ondernemend_veld`='$this->ondernemend',`AVO_veld`='$this->AVO',`softskills_veld`=' $this->softskills',`evtKwaliteiten_veld`='$this->evt_kwaliteiten',`doorgroei_advies`='$this->doorgroei_advies' WHERE leerlingnummer = '$_leerlingNummer'");
            if ($this->mysqliQRY) {
